#pragma once

#include "../SDK.h"

// Name: Arise, Version: 1.0.0

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

namespace SDK
{
//---------------------------------------------------------------------------
// Parameters
//---------------------------------------------------------------------------

// Function BP_USS_ROD_EXTENSION.BP_USS_ROD_EXTENSION_C.OnExtensionRankChanged
struct UBP_USS_ROD_EXTENSION_C_OnExtensionRankChanged_Params
{
};

// Function BP_USS_ROD_EXTENSION.BP_USS_ROD_EXTENSION_C.OnEnabled
struct UBP_USS_ROD_EXTENSION_C_OnEnabled_Params
{
};

// Function BP_USS_ROD_EXTENSION.BP_USS_ROD_EXTENSION_C.OnDisabled
struct UBP_USS_ROD_EXTENSION_C_OnDisabled_Params
{
};

// Function BP_USS_ROD_EXTENSION.BP_USS_ROD_EXTENSION_C.ExecuteUbergraph_BP_USS_ROD_EXTENSION
struct UBP_USS_ROD_EXTENSION_C_ExecuteUbergraph_BP_USS_ROD_EXTENSION_Params
{
	int                                                EntryPoint;                                               // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
